...
/*
	...
	-End Condition: Quando as peças tiverem  sido todas jogadas
	-Winning Conditions: Jogador com mais pontos ganha
...
void bubbleSort(vector<Person>& v, bool f(int x, int y))       //To sort the players by their points              
...
void verifyPlayerNum(int& numPlayers, Pool pool)   //Checks if it is possible to play with the chosen board with the selected amount of players(ex: 4 players and the pool has 15 pieces -> Impossible )
{
	...
	while (true)
	{
		...
		else if (pool.showNumber() < 14)
		{
			cout << "The board chosen doesn't have enough pieces to allow two players to play, therefore it is invalid!" << endl;
		}
		...
		{
			cout << "The minimum ammount of players is two and the maximum is four. Please retry!" << endl;
		}
		else
			break;
	}
...
void hasNoMoves(int& counter, Person& p1, Pool& pool, bool& playedonce)   //The player has no possible moves so he refills from the pool
{
	...
	for (int i = 0; i < MAXEXCHANGPIECES; i++) //asks for a piece to exhange in the pool twice
	{
		if (pool.showNumber() == 0) //if the pool is empty passes
		{
			...
			cout << "Press any key to pass..." << endl;
			cin.ignore();
	...
	else if (numPlayers == 3)
	{
		...
		else
		{
			...
			if (pointOrder[2].showpoints() == pointOrder[1].showpoints())
			{
				...
				cout << pointOrder[1].showname() << " got second place with" << pointOrder[1].showpoints() << " points!" << endl;
		}
	}
	else
	{
		...
		{
			cout << "The game ended in a draw with players " << pointOrder[0].showname() << ", " << pointOrder[1].showname() << "and" << pointOrder[2].showname() << "finishing with " << pointOrder[0].showpoints() << " points!" << endl;
		}
		else if (pointOrder[0].showpoints() == pointOrder[1].showpoints() && pointOrder[0].showpoints() != pointOrder[2].showpoints() && pointOrder[0].showpoints() != pointOrder[3].showpoints())
		{
			cout << "The game ended in a draw with players " << pointOrder[0].showname() << " and " << pointOrder[1].showname() << "finishing with " << pointOrder[0].showpoints() << " points!" << endl;
			if (pointOrder[3].showpoints() == pointOrder[2].showpoints())
			{
				cout << "The ramaing two players ended up in a draw for second place with " << pointOrder[2].showpoints() << " points each!" << endl;
			}
			else
			{
				cout << pointOrder[2].showname() << " got second place with " << pointOrder[2].showpoints() << " points!" << endl;
			}
		}
		else
		{
...
int main() {
	...
	//game-------
	Person p1(1), p2(2), p3(3), p4(4); //inicialization of 4 default players (they are inicialized with and int that represents the player order)
	...
	// GAME ----------
	while (p1.showHandSize() != 0 || p2.showHandSize() != 0 || p3.showHandSize() != 0 || p4.showHandSize() != 0) //End condition (no more pieces in the hands mean that there are no pieces in the pool either so all the pieces have been placed in the board)
	{
