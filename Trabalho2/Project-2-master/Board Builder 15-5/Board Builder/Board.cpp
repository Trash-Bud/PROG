...
bool Board::isValid(string input)
{
	...
	while (dictionary >> content)
	{
		if (word == content)
		{
			...
			{
				if (words[y].substr(5) == word)
				{
					cout << "The word is already in the board!" << endl;
					Sleep(1000);
					return false;
				}
			}
...
void Board::print()
{
	...
	while (true)
	{
		...
		{
			cout << "This action will overwrite the default board. Do you wish to continue? (type 'yes' for replacing or any key to choose another name)" << endl;
			cin >> option;
